      ...
      , _totalReadAttempts(0)
{
    ...
    // Alokacja tablicy na sumy temperatur (bieżący cykl 30 pomiarów)
...
float DS18B20MultiPin::getTemperature(uint8_t pinIndex) const
{
    ...
    // Zwróć ostatnią obliczoną średnią (aktualizowaną co 30 pomiarów)
...
void DS18B20MultiPin::_readAll()
{
    ...
    // Jeżeli osiągnięto 30 prób, oblicz średnie i resetuj liczniki
    if (_totalReadAttempts >= MAX_READINGS)
    {
        DPRINTLN(F("[DS18B20MultiPin] *** Completing 30-read cycle, updating averages ***"));
